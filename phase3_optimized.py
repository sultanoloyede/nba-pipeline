"""
Phase 3: Fast Model Training (OPTIMIZED - Using Pre-calculated Percentages)

This is the OPTIMIZED implementation following OPTIMIZED_PIPELINE_PLAN.md.

Key Optimizations:
1. Downloads comprehensive file with ALL threshold columns ONCE (not 42 times)
2. Reuses the DataFrame for all 42 models
3. Just filters columns per threshold (no repeated downloads)
4. Pre-calculated percentages make training ~10x faster
5. Expected time: 30-35 minutes for all 42 models
...
def run_phase_3_optimized(s3_handler, threshold_start: int = 10, threshold_end: int = 51) -> Tuple[bool, Dict]:
    ...
    Prerequisites:
    - Phase 2.5 must be completed (pre-calculated percentage file must exist)
    ...
    try:
        ...
        # ============================================================================
        # STEP 1: Load Comprehensive Data (ONCE)
        # ============================================================================
        ...
        if df_all is None:
            error_msg = (
                f"Failed to download pre-calculated data from S3.\n"
                f"  Expected file: s3://{S3_PLAYER_BUCKET}/{data_key}\n"
                f"  Please run Phase 2.5 first to generate this file."
            )
            logger.error(error_msg)
            return False, {'error': error_msg}

        logger.info(f"✓ Loaded comprehensive data:")
        ...
        # Check specifically for threshold 10
        ...
        # ============================================================================
        logger.info("\nSTEP 2: Training models for each threshold...")
