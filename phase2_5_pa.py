...
This phase runs to pre-calculate percentage columns for PA thresholds (5-39)
...
def calculate_rolling_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int], window: int) -> Dict[int, np.ndarray]:
    ...
    Args:
        ...
        thresholds: List of PA thresholds to calculate percentages for
    ...
    for group_idx, (name, group) in enumerate(grouped):
        ...
        for threshold in thresholds:
            # Vectorized: Create boolean array where PA >= threshold
            met_threshold = (group_sorted['PA'].values >= threshold).astype(float)
...
def calculate_season_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int]) -> Dict[int, np.ndarray]:
    ...
    Args:
        df: DataFrame with RA, Player_ID, TEAM, SEASON_ID, GAME_DATE_PARSED
        thresholds: List of PA thresholds
...
def calculate_last_season_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int]) -> Dict[int, np.ndarray]:
    ...
    for threshold in thresholds:
        ...
        season_pct = df.groupby(['Player_ID', 'SEASON_ID']).apply(
            lambda group: (group['PA'] >= threshold).mean()
        ).reset_index()
        ...
        for threshold in thresholds:
            ...
            for i, (idx, pct_val) in enumerate(zip(pt_indices, pt_pct_values)):
                if np.isnan(pct_val):
                    ...
                    # Add current game's PA to the list for future first-lineup-games
                    current_ra = pt_group.iloc[i]['PA']
    ...
    for group_idx, ((player_id, opponent), group) in enumerate(grouped):
        ...
        for row_pos in range(len(group_sorted)):
            ...
            if len(prev_games) > 0:
                ...
                for threshold in thresholds:
                    pct = (prev_games['PA'] >= threshold).mean()
...
def run_phase_2_5_pa(s3_handler, threshold_start=5, threshold_end=40, force_reprocess=False) -> Tuple[bool, dict]:
    """
    ...
    Args:
        ...
        threshold_end: Ending threshold + 1 (default: 40, so thresholds 5-39)
    ...
    """
    logger.info("=" * 80)
    logger.info("PHASE 2.5 PA: PRE-CALCULATE ALL PERCENTAGE COLUMNS (OPTIMIZED + METADATA)")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Step 1: Download processed PA data from Phase 2
        logger.info("Step 1: Downloading processed PA data from Phase 2...")
        ...
        if df is None:
            logger.error("Failed to download processed PA data from S3")
            return False, {'error': 'Failed to download processed PA data'}

        logger.info(f"✓ Downloaded processed PA data: {len(df):,} rows")
        ...
        metadata_key = 'metadata/pa_pct_metadata.json'
        if not force_reprocess:
            ...
            if not needs_processing:
                logger.info("\n" + "=" * 80)
                logger.info("PHASE 2.5 PA SKIPPED - Already up to date")
                logger.info("=" * 80)
        ...
        # Step 3: Save to S3
        logger.info("\n[6/7] Saving pre-calculated PA data to S3...")
        output_key = f'processed_data_pa/processed_with_pa_pct_{threshold_start}-{threshold_end - 1}.csv'
        ...
        metadata = {
            ...
            'input_rows': len(df),
            'input_file': 'processed_data_pa/processed_model_data_pa.csv',
        ...
        logger.info("\n" + "=" * 80)
        logger.info("PHASE 2.5 PA SUMMARY")
        logger.info("=" * 80)
        ...
        logger.info("=" * 80)
        logger.info("✓ PHASE 2.5 PA COMPLETED SUCCESSFULLY")
        logger.info("=" * 80)

        return True, stats

    except Exception as e:
        logger.error(f"Phase 2.5 PA failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    success, stats = run_phase_2_5_pa(s3_handler, force_reprocess=False)

    if success:
        if stats.get('skipped'):
            print("\n→ Phase 2.5 PA skipped (already up to date)")
            print(f"Reason: {stats.get('reason')}")
        else:
            print("\n✓ Phase 2.5 PA completed successfully!")
            print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 2.5 PA failed!")
