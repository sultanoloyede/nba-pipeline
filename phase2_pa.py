"""
Phase 2 PA: Vectorized Data Processing for PA (Points + Assists)

This module downloads all player files from S3, combines them, performs
feature engineering using 100% VECTORIZED operations for PA stat,
and saves the processed data back to S3.

Key Differences from PRA:
- Uses PA (PTS + AST) instead of PRA
- Saves to processed_data_pa/ folder
- All base features calculated from PA values
...
def calculate_rolling_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, GAME_DATE_PARSED, PA columns
    ...
    def rolling_for_group(group):
        ...
        group['last_5_avg'] = group['PA'].rolling(5, min_periods=1).mean().shift(1)
        group['last_10_avg'] = group['PA'].rolling(10, min_periods=1).mean().shift(1)
        group['last_20_avg'] = group['PA'].rolling(20, min_periods=1).mean().shift(1)
        return group
...
def calculate_season_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, PA columns
    ...
    # Current season average (expanding mean excluding current game)
    df['season_avg'] = df.groupby(['Player_ID', 'TEAM', 'SEASON_ID'])['PA'].transform(
...
def calculate_lineup_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    For first game with lineup, use average PA from all previous "first games with new lineups".

    Args:
        df: DataFrame with Player_ID, TEAM, LINEUP_ID, PA columns
    ...
    # For each player-team-lineup combo, calculate expanding average
    df['lineup_average'] = df.groupby(['Player_ID', 'TEAM', 'LINEUP_ID'])['PA'].transform(
        lambda x: x.expanding().mean().shift(1)
    )

    # For first game with lineup (NaN), use "new lineup average"
    # This is the average PA from all previous "first games with new lineups"
        ...
        def calc_new_lineup_avg(group):
            ...
            for idx in range(len(group)):
                if group.iloc[idx]['is_first_lineup_game'] == 1:
                    ...
                    # Add this game's PA to the list for future first-lineup-games
                    first_game_ras.append(group.iloc[idx]['PA'])
                else:
...
def calculate_h2h_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, OPPONENT, SEASON_ID, PA columns
    ...
    def h2h_for_seasons(group):
        ...
        for idx in range(len(group)):
            ...
            h2h_avg = prev_games['PA'].mean() if len(prev_games) > 0 else 0.0
...
def calculate_opponent_strength_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def opp_strength_for_season(group):
        ...
        # went_under = 1 if PA < floor(last_5_avg), 0 otherwise
        group['went_under'] = (group['PA'] < np.floor(group['last_5_avg'])).astype(float)
...
def run_phase_2_pa(s3_handler) -> Tuple[bool, dict]:
    """
    Execute Phase 2 PA: Data Processing for PA (Points + Assists).
    ...
    4. Saves processed data to S3 in separate PA folder
    ...
    """
    logger.info("=" * 80)
    logger.info("PHASE 2 PA: DATA PROCESSING (OPTIMIZED - VECTORIZED)")
    logger.info("=" * 80)
    ...
    try:
        ...
        player_files = [
            ...
            and not f.startswith('processed_data_pa/')
        ...
        # Step 10: Save to S3 in PA folder
        logger.info("\nStep 10: Saving processed PA data to S3...")
        s3_handler.upload_dataframe(
            combined_df,
            S3_PLAYER_BUCKET,
            'processed_data_pa/processed_model_data_pa.csv'
        )
        logger.info("  ✓ Saved to s3://deviation-io-player-bucket/processed_data_pa/processed_model_data_pa.csv")
        ...
        stats = {
            ...
            'features_added': [
                'PA', 'GAME_DATE_PARSED', 'TEAM', 'OPPONENT',
            ...
            'stat_type': 'PA (Points + Assists)'
        }

        logger.info("\n" + "=" * 80)
        logger.info("PHASE 2 PA SUMMARY (OPTIMIZED)")
        logger.info("=" * 80)
        ...
        logger.error(f"Phase 2 PA failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    success, stats = run_phase_2_pa(s3_handler)

    if success:
        print("\n✓ Phase 2 PA (OPTIMIZED) completed successfully!")
        print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 2 PA failed!")
