"""
Phase 2: Vectorized Data Processing (OPTIMIZED)

This module downloads all player files from S3, combines them, performs
feature engineering using 100% VECTORIZED operations, and saves the processed data back to S3.
...
AI-generated code: This script processes NBA player game log data by combining
individual player files and calculating various statistical features using
...
def calculate_rolling_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, GAME_DATE_PARSED, PRA columns
    ...
    def rolling_for_group(group):
        ...
        group['last_5_avg'] = group['PRA'].rolling(5, min_periods=1).mean().shift(1)
        group['last_10_avg'] = group['PRA'].rolling(10, min_periods=1).mean().shift(1)
        group['last_20_avg'] = group['PRA'].rolling(20, min_periods=1).mean().shift(1)
        return group
...
def calculate_season_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, PRA columns
    ...
    # Current season average (expanding mean excluding current game)
    df['season_avg'] = df.groupby(['Player_ID', 'TEAM', 'SEASON_ID'])['PRA'].transform(
    ...
    season_stats = df.groupby(['Player_ID', 'SEASON_ID'])['PRA'].mean().reset_index()
...
def calculate_lineup_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    For first game with lineup, use average PRA from all previous "first games with new lineups".

    Args:
        df: DataFrame with Player_ID, TEAM, LINEUP_ID, PRA columns
    ...
    # For each player-team-lineup combo, calculate expanding average
    df['lineup_average'] = df.groupby(['Player_ID', 'TEAM', 'LINEUP_ID'])['PRA'].transform(
        lambda x: x.expanding().mean().shift(1)
    )

    # For first game with lineup (NaN), use "new lineup average"
    # This is the average PRA from all previous "first games with new lineups"
        ...
        def calc_new_lineup_avg(group):
            ...
            for idx in range(len(group)):
                if group.iloc[idx]['is_first_lineup_game'] == 1:
                    ...
                    if len(first_game_pras) > 0:
                        # Use average of previous first-lineup-games
                        new_lineup_avgs.append(np.mean(first_game_pras))
                    else:
                        # No previous first-lineup-games, use 0
                        new_lineup_avgs.append(0.0)
                    # Add this game's PRA to the list for future first-lineup-games
                    first_game_pras.append(group.iloc[idx]['PRA'])
                else:
...
def calculate_h2h_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, OPPONENT, SEASON_ID, PRA columns
    ...
    def h2h_for_seasons(group):
        ...
        for idx in range(len(group)):
            ...
            h2h_avg = prev_games['PRA'].mean() if len(prev_games) > 0 else 0.0
...
def calculate_opponent_strength_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with OPPONENT, SEASON_ID, PRA, last_5_avg columns
    ...
    def opp_strength_for_season(group):
        ...
        # went_under = 1 if PRA < floor(last_5_avg), 0 otherwise
        group['went_under'] = (group['PRA'] < np.floor(group['last_5_avg'])).astype(float)
...
def run_phase_2_optimized(s3_handler) -> Tuple[bool, dict]:
    """
    Execute Phase 2: Data Processing (OPTIMIZED WITH VECTORIZATION).
    ...
    """
    logger.info("=" * 80)
    logger.info("PHASE 2: DATA PROCESSING (OPTIMIZED - VECTORIZED)")
    logger.info("=" * 80)
    ...
    try:
        ...
        # PRA (vectorized)
        if 'PRA' not in combined_df.columns:
        ...
        # Step 10: Save to S3
        logger.info("\nStep 10: Saving processed data to S3...")
        s3_handler.upload_dataframe(
            combined_df,
            S3_PLAYER_BUCKET,
            'processed_data/processed_model_data.csv'
        )
        logger.info("  ✓ Saved to s3://deviation-io-player-bucket/processed_data/processed_model_data.csv")
        ...
        logger.info("\n" + "=" * 80)
        logger.info("PHASE 2 SUMMARY (OPTIMIZED)")
        logger.info("=" * 80)
...
run_phase_2 = run_phase_2_optimized
...
if __name__ == '__main__':
    ...
    success, stats = run_phase_2_optimized(s3_handler)

    if success:
        print("\n✓ Phase 2 (OPTIMIZED) completed successfully!")
        print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 2 failed!")
