"""
Phase 3 RA: Fast Model Training for RA (OPTIMIZED - Using Pre-calculated Percentages)

This is the RA-specific implementation for training models on RA (Rebounds + Assists).

Key Optimizations:
1. Downloads comprehensive RA file with ALL threshold columns ONCE (not 22 times)
2. Reuses the DataFrame for all 22 models (thresholds 5-26)
3. Just filters columns per threshold (no repeated downloads)
4. Pre-calculated percentages make training ~10x faster
5. Expected time: 15-20 minutes for all 22 models
...
def train_model_for_threshold(df_all: pd.DataFrame, threshold: int) -> Tuple[xgb.XGBClassifier, float, Dict]:
    """
    Train XGBoost model for specific RA threshold.

    Uses pre-loaded comprehensive DataFrame, just filters columns.

    Args:
        df_all: Comprehensive DataFrame with all RA percentage columns
        threshold: RA threshold for binary classification
    ...
    y = (df_all['RA'] >= threshold).astype(int)
...
def run_phase_3_ra(s3_handler, threshold_start: int = 5, threshold_end: int = 26) -> Tuple[bool, Dict]:
    """
    Execute Phase 3 RA (OPTIMIZED): Model Training for RA using pre-calculated percentages.

    This is the RA-specific implementation that:
    1. Downloads comprehensive RA file ONCE
    2. Reuses DataFrame for all 22 models (thresholds 5-26)
    3. Trains models sequentially (one threshold at a time)
    4. Saves each model to S3 immediately in separate RA folder

    Prerequisites:
    - Phase 2.5 RA must be completed (pre-calculated percentage file must exist)

    Args:
        s3_handler: S3Handler instance
        threshold_start: Starting threshold (default: 5 for RA)
        threshold_end: Ending threshold (default: 26 for RA, so thresholds 5-26 inclusive)

    Returns:
        Tuple of (success: bool, stats: dict)
    """
    logger.info("=" * 80)
    logger.info("PHASE 3 RA (OPTIMIZED): FAST MODEL TRAINING FOR RA")
    logger.info("=" * 80)
    ...
    try:
        ...
        # ============================================================================
        # STEP 1: Load Comprehensive RA Data (ONCE)
        # ============================================================================
        logger.info("\nSTEP 1: Loading comprehensive RA data with all percentage columns...")

        data_key = f'processed_data_ra/processed_with_ra_pct_{threshold_start}-{threshold_end}.csv'
        ...
        if df_all is None:
            error_msg = (
                f"Failed to download pre-calculated RA data from S3.\n"
                f"  Expected file: s3://{S3_PLAYER_BUCKET}/{data_key}\n"
                f"  Please run Phase 2.5 RA first to generate this file."
            )
            logger.error(error_msg)
            return False, {'error': error_msg}

        logger.info(f"✓ Loaded comprehensive RA data:")
        ...
        required_base_cols = ['RA', 'last_5_avg', 'last_10_avg', 'last_20_avg', 'season_avg']
        ...
        # ============================================================================
        logger.info("\nSTEP 2: Training models for each RA threshold...")
        ...
        for threshold in range(threshold_start, threshold_end + 1):
            ...
            # Save model to S3 in RA folder
            logger.info(f"  Saving model to S3...")
            model_filename = f'xgb_ra_{threshold}plus_precision_{precision:.4f}.pkl'
            model_key = f'models_ra/{model_filename}'
        ...
        # ============================================================================
        logger.info("\n" + "=" * 80)
        logger.info("PHASE 3 RA (OPTIMIZED) - SUMMARY")
        logger.info("=" * 80)
        logger.info(f"Stat Type: RA (Rebounds + Assists)")
        ...
        logger.info("=" * 80)
        logger.info("✓ PHASE 3 RA COMPLETED SUCCESSFULLY")
        logger.info("=" * 80)
        ...
        logger.error(f"Phase 3 RA (Optimized) failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    # Train RA models (thresholds 5-26)
    print("\nTraining RA models (thresholds 5-26)...")
    success, stats = run_phase_3_ra(s3_handler, threshold_start=5, threshold_end=26)

    if success:
        print("\n✓ Phase 3 RA (Optimized) completed successfully!")
        print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 3 RA (Optimized) failed!")
