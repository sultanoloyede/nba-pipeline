"""
Phase 2 RA: Vectorized Data Processing for RA (Rebounds + Assists)

This module downloads all player files from S3, combines them, performs
feature engineering using 100% VECTORIZED operations for RA stat,
and saves the processed data back to S3.

Key Differences from PRA:
- Uses RA (REB + AST) instead of PRA
- Saves to processed_data_ra/ folder
- All base features calculated from RA values
...
def calculate_rolling_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, GAME_DATE_PARSED, RA columns
    ...
    def rolling_for_group(group):
        ...
        group['last_5_avg'] = group['RA'].rolling(5, min_periods=1).mean().shift(1)
        group['last_10_avg'] = group['RA'].rolling(10, min_periods=1).mean().shift(1)
        group['last_20_avg'] = group['RA'].rolling(20, min_periods=1).mean().shift(1)
        return group
...
def calculate_season_averages_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, TEAM, SEASON_ID, RA columns
    ...
    # Current season average (expanding mean excluding current game)
    df['season_avg'] = df.groupby(['Player_ID', 'TEAM', 'SEASON_ID'])['RA'].transform(
    ...
    season_stats = df.groupby(['Player_ID', 'SEASON_ID'])['RA'].mean().reset_index()
...
def calculate_lineup_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    For first game with lineup, use average RA from all previous "first games with new lineups".

    Args:
        df: DataFrame with Player_ID, TEAM, LINEUP_ID, RA columns
    ...
    # For each player-team-lineup combo, calculate expanding average
    df['lineup_average'] = df.groupby(['Player_ID', 'TEAM', 'LINEUP_ID'])['RA'].transform(
        lambda x: x.expanding().mean().shift(1)
    )

    # For first game with lineup (NaN), use "new lineup average"
    # This is the average RA from all previous "first games with new lineups"
        ...
        def calc_new_lineup_avg(group):
            ...
            for idx in range(len(group)):
                if group.iloc[idx]['is_first_lineup_game'] == 1:
                    ...
                    # Add this game's RA to the list for future first-lineup-games
                    first_game_ras.append(group.iloc[idx]['RA'])
                else:
...
def calculate_h2h_average_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame with Player_ID, OPPONENT, SEASON_ID, RA columns
    ...
    def h2h_for_seasons(group):
        ...
        for idx in range(len(group)):
            ...
            h2h_avg = prev_games['RA'].mean() if len(prev_games) > 0 else 0.0
...
def calculate_opponent_strength_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def opp_strength_for_season(group):
        ...
        # went_under = 1 if RA < floor(last_5_avg), 0 otherwise
        group['went_under'] = (group['RA'] < np.floor(group['last_5_avg'])).astype(float)
...
def calculate_games_missed_vectorized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['team_game_number'] = (
        ...
        .ngroup()
        .pipe(lambda x: df.groupby(['TEAM', 'SEASON_ID']).cumcount() + 1)
    )

    # Actually, let's use a simpler approach - rank games by date within each team-season
    df['team_game_number'] = (
        df.groupby(['TEAM', 'SEASON_ID'])['GAME_DATE_PARSED']
...
def run_phase_2_ra(s3_handler) -> Tuple[bool, dict]:
    """
    Execute Phase 2 RA: Data Processing for RA (Rebounds + Assists).
    ...
    4. Saves processed data to S3 in separate RA folder
    ...
    """
    logger.info("=" * 80)
    logger.info("PHASE 2 RA: DATA PROCESSING (OPTIMIZED - VECTORIZED)")
    logger.info("=" * 80)
    ...
    try:
        ...
        player_files = [
            ...
            and not f.startswith('processed_data_ra/')
        ...
        # Step 9: Calculate games missed (VECTORIZED)
        logger.info("\nStep 9: Calculating games missed (vectorized)...")
        combined_df = calculate_games_missed_vectorized(combined_df)

        # Step 10: Save to S3 in RA folder
        logger.info("\nStep 10: Saving processed RA data to S3...")
        s3_handler.upload_dataframe(
            combined_df,
            S3_PLAYER_BUCKET,
            'processed_data_ra/processed_model_data_ra.csv'
        )
        logger.info("  ✓ Saved to s3://deviation-io-player-bucket/processed_data_ra/processed_model_data_ra.csv")
        ...
        stats = {
            ...
            'features_added': [
                'RA', 'GAME_DATE_PARSED', 'TEAM', 'OPPONENT',
            ...
            'stat_type': 'RA (Rebounds + Assists)'
        }

        logger.info("\n" + "=" * 80)
        logger.info("PHASE 2 RA SUMMARY (OPTIMIZED)")
        logger.info("=" * 80)
        ...
        logger.error(f"Phase 2 RA failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    success, stats = run_phase_2_ra(s3_handler)

    if success:
        print("\n✓ Phase 2 RA (OPTIMIZED) completed successfully!")
        print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 2 RA failed!")
