"""
Phase 2.5 PR: Pre-calculate ALL Percentage Columns for PR (OPTIMIZED + METADATA)

This phase runs to pre-calculate percentage columns for PR thresholds (5-39)
...
def calculate_rolling_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int], window: int) -> Dict[int, np.ndarray]:
    ...
    for group_idx, (name, group) in enumerate(grouped):
        ...
        for threshold in thresholds:
            # Vectorized: Create boolean array where PR >= threshold
            met_threshold = (group_sorted['PR'].values >= threshold).astype(float)
...
def calculate_season_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int]) -> Dict[int, np.ndarray]:
    ...
    for threshold in thresholds:
        # Create boolean column where PR >= threshold
        met_threshold = (df['PR'] >= threshold).astype(float)
...
def calculate_last_season_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int]) -> Dict[int, np.ndarray]:
    ...
    for threshold in thresholds:
        ...
        season_pct = df.groupby(['Player_ID', 'SEASON_ID']).apply(
            lambda group: (group['PR'] >= threshold).mean()
        ).reset_index()
    ...
    for group_idx, ((player_id, opponent), group) in enumerate(grouped):
        ...
        for row_pos in range(len(group_sorted)):
            ...
            if len(prev_games) > 0:
                ...
                for threshold in thresholds:
                    pct = (prev_games['PR'] >= threshold).mean()
...
def run_phase_2_5_pr(s3_handler, threshold_start=5, threshold_end=40, force_reprocess=False) -> Tuple[bool, dict]:
    """
    ...
    """
    logger.info("=" * 80)
    logger.info("PHASE 2.5 PR: PRE-CALCULATE ALL PERCENTAGE COLUMNS (OPTIMIZED + METADATA)")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Step 1: Download processed PR data from Phase 2
        ...
        if df is None:
            logger.error("Failed to download processed PR data from S3")
            return False, {'error': 'Failed to download processed PR data'}

        logger.info(f"✓ Downloaded processed PR data: {len(df):,} rows")
        ...
        logger.info("\n" + "=" * 80)
        logger.info("PHASE 2.5 PR SUMMARY")
        logger.info("=" * 80)
        ...
        logger.error(f"Phase 2.5 PR failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    success, stats = run_phase_2_5_pr(s3_handler, force_reprocess=False)

    if success:
        if stats.get('skipped'):
            print("\n→ Phase 2.5 PR skipped (already up to date)")
            print(f"Reason: {stats.get('reason')}")
        else:
            print("\n✓ Phase 2.5 PR completed successfully!")
            print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 2.5 PR failed!")
