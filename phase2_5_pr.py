...
This phase runs to pre-calculate percentage columns for PR thresholds (8-40)
...
def calculate_season_percentage_all_thresholds_vectorized(df: pd.DataFrame, thresholds: List[int]) -> Dict[int, np.ndarray]:
    ...
    for threshold in thresholds:
        # Create boolean column where PR >= threshold (on sorted df)
        met_threshold = (df_sorted['PR'] >= threshold).astype(float)
