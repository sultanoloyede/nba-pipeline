"""
Phase 3 PA: Fast Model Training for PA (OPTIMIZED - Using Pre-calculated Percentages)

This is the RA-specific implementation for training models on PA (Points + Assists).

Key Optimizations:
1. Downloads comprehensive PA file with ALL threshold columns ONCE (not 34 times)
...
def train_model_for_threshold(df_all: pd.DataFrame, threshold: int) -> Tuple[xgb.XGBClassifier, float, Dict]:
    """
    Train XGBoost model for specific PA threshold.

    Uses pre-loaded comprehensive DataFrame, just filters columns.

    Args:
        df_all: Comprehensive DataFrame with all PA percentage columns
        threshold: PA threshold for binary classification
    ...
    y = (df_all['PA'] >= threshold).astype(int)
...
def run_phase_3_pa(s3_handler, threshold_start: int = 8, threshold_end: int = 41) -> Tuple[bool, Dict]:
    """
    ...
    Prerequisites:
    - Phase 2.5 PA must be completed (pre-calculated percentage file must exist)

    Args:
        s3_handler: S3Handler instance
        threshold_start: Starting threshold (default: 8 for PA, representing o/u 7.5)
        threshold_end: Ending threshold (default: 41 for PA, so thresholds 8-40 inclusive representing o/u 7.5-40.5)

    Returns:
        Tuple of (success: bool, stats: dict)
    """
    logger.info("=" * 80)
    logger.info("PHASE 3 PA (OPTIMIZED): FAST MODEL TRAINING FOR RA")
    logger.info("=" * 80)
    logger.info(f"Stat Type: PA (Points + Assists)")
    ...
    try:
        ...
        # ============================================================================
        # STEP 1: Load Comprehensive PA Data (ONCE)
        # ============================================================================
        logger.info("\nSTEP 1: Loading comprehensive PA data with all percentage columns...")

        data_key = f'processed_data_pa/processed_with_pa_pct_{threshold_start}-{threshold_end - 1}.csv'
        ...
        if df_all is None:
            error_msg = (
                f"Failed to download pre-calculated PA data from S3.\n"
                f"  Expected file: s3://{S3_PLAYER_BUCKET}/{data_key}\n"
                f"  Please run Phase 2.5 PA first to generate this file."
            )
            logger.error(error_msg)
            return False, {'error': error_msg}

        logger.info(f"✓ Loaded comprehensive PA data:")
        ...
        required_base_cols = ['PA', 'last_5_avg', 'last_10_avg', 'last_20_avg', 'season_avg']
        ...
        # ============================================================================
        logger.info("\nSTEP 2: Training models for each PA threshold...")
        ...
        for threshold in range(threshold_start, threshold_end + 1):
            ...
            # Save model to S3 in PA folder
            logger.info(f"  Saving model to S3...")
            model_filename = f'xgb_pa_{threshold}plus_precision_{precision:.4f}.pkl'
            model_key = f'models_pa/{model_filename}'
        ...
        # ============================================================================
        logger.info("\n" + "=" * 80)
        logger.info("PHASE 3 PA (OPTIMIZED) - SUMMARY")
        logger.info("=" * 80)
        logger.info(f"Stat Type: PA (Points + Assists)")
        ...
        logger.info("=" * 80)
        logger.info("✓ PHASE 3 PA COMPLETED SUCCESSFULLY")
        logger.info("=" * 80)
        ...
        logger.error(f"Phase 3 PA (Optimized) failed with error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    # Train PA models (thresholds 8-41, representing o/u 7.5-40.5)
    print("\nTraining PA models (thresholds 8-41)...")
    success, stats = run_phase_3_pa(s3_handler, threshold_start=8, threshold_end=41)

    if success:
        print("\n✓ Phase 3 PA (Optimized) completed successfully!")
        print(f"Stats: {stats}")
    else:
        print("\n✗ Phase 3 PA (Optimized) failed!")
